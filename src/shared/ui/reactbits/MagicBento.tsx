...
/**
 * Ported from React Bits (MagicBento), converted to TypeScript and made
 * theme-aware: the glow defaults to the app's live `--accent` (as an "r, g, b"
 * string) instead of the original hard-coded purple, and the card content is
 * prop-driven so it can carry real Zen content.
 */
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor }: GlobalSpotlightProps) => {
    ...
    <>
      ...
      <div className={`card-grid bento-section ${calm ? "magic-bento--calm" : ""} ${className}`} ref={gridRef}>
        {cards.map((card, index) => {
          ...
          return (
            <div key={index} className={cls} style={style} onClick={card.onClick}>
              {content}
            </div>
          );
        })}
