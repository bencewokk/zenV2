...
/// Load the OAuth client id + secret. Looks at env vars first, then a JSON file
/// (`google_oauth.json`) in the working dir or `src-tauri/`. See SETUP.md.
fn load_credentials() -> Result<OAuthCredentials, String> {
    ...
    Err("No Google OAuth credentials. Set ZEN_GOOGLE_CLIENT_ID / \
ZEN_GOOGLE_CLIENT_SECRET, or create google_oauth.json (see SETUP.md)."
        .into())
}
